...
for(i in 1:n.loci){
	...
	relate.string <- paste("../../relate 
	                       --mode All
	                       -m", as.character(u),
	                       "-N", as.character(N*2),
	                       "--haps", relate_haps,
	                       "--sample", relate_sample,
	                       "--map", relate_map,
	                       "--output", relate_out)
  system(relate.string)
	...
	extract_tree <- paste("../../RelateExtract --mode AncToNewick",
	                      "--anc data.anc", 
	                      "--mut", as.character(u),
	...
	relate_trees_list[[i]] <- read.tree(text = readLines(paste(relate_out, ".newick", sep = "")))
...
anc_trees_relate <- list()
der_trees_relate <- list()
ct_relate_all <- list()
ct_relate_anc <- list()
ct_relate_der <- list()
...
for(i in 1:n.loci){
	...
	anc_tips_relate <- which(relate_trees_list[[i]]$tip.label %in% as.character(1:(n_chroms - n_ders[i])))
	...
	der_tips_relate <- which(relate_trees_list[[i]]$tip.label %in% as.character((n_chroms - n_ders[i] + 1):n_chroms))
	...
	anc_trees_relate[[i]] <- drop.tip(relate_trees_list[[i]], der_tips_relate)
	der_trees_relate[[i]] <- drop.tip(relate_trees_list[[i]], anc_tips_relate)
	...
	ct_relate_all[[i]] <- coal.times(relate_trees_list[[i]])
	ct_relate_anc[[i]] <- coal.times(anc_trees_relate[[i]])
  ct_relate_der[[i]] <- coal.times(der_trees_relate[[i]])	
...
for(i in 1:length(relate_trees_list)){
  times.c.relate[[i]] <- trees_to_times(relate_trees_list[[i]], anc_trees_relate[[i]], der_trees_relate[[i]], time, sure.alt.is.derived = FALSE, units_in = 2)
}

lins.list.relqte <- list()
for(i in 1:length(relate_trees_list)){
